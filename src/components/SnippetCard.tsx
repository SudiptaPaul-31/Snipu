...
export const SnippetCard: React.FC<SnippetCardProps> = ({ snippet, viewMode }) => {
  ...
  const codePreviewClasses =
    viewMode === "grid"
      ? `bg-gradient-to-r ${getLanguageGradient(snippet.language)} backdrop-blur-sm border border-slate-600/30 rounded-xl p-4 mb-6 overflow-hidden h-[100px] relative`
      : `bg-gradient-to-r ${getLanguageGradient(snippet.language)} backdrop-blur-sm border border-slate-600/30 rounded-xl p-4 mb-6 overflow-hidden h-[100px] relative flex-1`
        ...
        <div className={contentClasses}>
          <div className="flex-1 min-w-0 relative z-10">
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              >
                <span
                  className={`bg-gradient-to-r ${getLanguageGradient(snippet.language)} backdrop-blur-sm border border-slate-600/30 text-white px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-2 shadow-lg`}
                >
                  {getLanguageIcon(snippet.language)}
                  {snippet.language}
                </span>
                {snippet.isBookmarked && <Star className="w-4 h-4 text-yellow-400 fill-current" />}
              </motion.div>
            ...
            >
              ...
              <pre className="text-sm text-gray-300 whitespace-pre-wrap font-mono leading-relaxed">
                <code className="group-hover:text-gray-200 transition-colors">
                  {snippet.code.length > 120 ? `${snippet.code.substring(0, 120)}...` : snippet.code}
                </code>
              </pre>
            </motion.div>
